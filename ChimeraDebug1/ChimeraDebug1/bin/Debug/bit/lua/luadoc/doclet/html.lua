...
-- @param fname name of the function or table or ref to link to.
-- @param doc documentation table
-- @param kind String specifying the kind of element to link ("functions" or "tables").
...
function link_to (fname, doc, module_doc, file_doc, from, kind)
	...
	if kind == "ref" then
		-- assume external reference file
		local href = "refs/" .. modulename .. '.' .. fname
		string.gsub(from, "/", function () href = "../" .. href end)
		logger:info(string.format("link_to: reference to file `%s' from `%s'", modulename, from))
		logger:info(string.format("link_to: href: `%s'", href))
		return href
	end
...
--		logger:error(string.format("unresolved reference to function `%s': module `%s' not found", fname, modulename))
	...
	for _, func_name in pairs(module_doc[kind]) do
		if func_name == fname then
			logger:info(string.format("link_to: call ml: reference to file `%s' from `%s'", modulename, from))
...
function symbol_link (symbol, doc, module_doc, file_doc, from)
	...
	logger:info(string.format("symbol_link call: reference to symbol `%s'", symbol))

	local href = 
--		file_link(symbol, from) or
		module_link(symbol, doc, from) or 
		link_to(symbol, doc, module_doc, file_doc, from, "functions") or
		link_to(symbol, doc, module_doc, file_doc, from, "tables") or
		link_to(symbol, doc, module_doc, file_doc, from, "ref")
